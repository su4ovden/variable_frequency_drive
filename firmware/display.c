...
void display_print_char(char symbol, uint8_t position)
{
	...
	{
		display_data[position] = get_symbol(symbol - '0');
	}
	else if(symbol == ' ')
	{
	...
	{
		display_data[position - 1] |= 1 << 7;
	}
}

void display_print_string(char* string)
{
	uint8_t dot_pos_compensation = 0;
	char symbol = 0;
	
	for(int8_t i = 0; i < (4 + dot_pos_compensation); i++)
	{
		symbol = string[i];
		display_print_char(symbol, i - dot_pos_compensation);
		
		if(symbol == '.')
		{
			dot_pos_compensation++;
		}
	}
}
