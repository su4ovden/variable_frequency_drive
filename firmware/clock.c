...
void clock_init(void)
{
	...
	RCC->PLLCFGR |= RCC_PLLCFGR_PLLM & (25 << RCC_PLLCFGR_PLLM_Pos); 	// M coifficient -> 25 / 25 = 1 	MHz PLL input
	RCC->PLLCFGR |= RCC_PLLCFGR_PLLN & (336 << RCC_PLLCFGR_PLLN_Pos); // N coifficient -> 1 * 336 = 336 MHz PLL output
	RCC->PLLCFGR |= RCC_PLLCFGR_PLLP_0; 															// P coifficient -> 336 / 4 = 84 	MHz SYSCLK
	RCC->PLLCFGR |= RCC_PLLCFGR_PLLQ & (7 << RCC_PLLCFGR_PLLQ_Pos);  	// Q coifficient -> 336 / 7 = 48 	MHz Peripherial
